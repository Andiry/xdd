...
xdd_barrier(struct xdd_barrier *bp) {
     pthread_barrier_wait(&bp->theBarrier);
...
xdd_destroy_barrier(struct xdd_barrier *bp) {
	...
	char errmsg[256];
	uint16_t zeros[32] = {0};
	if (bp->initialized == FALSE) /* Barrier is already destroyed */
	     return;
	bp->initialized = FALSE;
	
	status = pthread_barrier_destroy(&bp->theBarrier);
	if (status) {
...
xdd_init_barrier(struct xdd_barrier *bp, int32_t threads, char *barrier_name) {
	...
	status = pthread_barrier_init(&bp->theBarrier, NULL, threads);
	if (status) {
